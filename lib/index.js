        ...
        .end(function(err, res) {
            ...
            if(self.swagger.basePath.indexOf('path') !== -1) {
                ...
                for(var apiIndex in self.swagger.apis) {
                    apis.push(new SwaggerMocha(self.baseURL, '/docs?path=' + self.swagger.apis[apiIndex].path));
                }
...
SwaggerMocha.prototype.getResults = function(callback) {
    ...
    apis.forEach(function(apiElement, apiIndex, apiArray) {
        ...
        apiElement.operations.forEach(function(operationElement, operationIndex, operationArray) {
            ...
            testedPaths.push({
                requestUrl: self.fullPath(self.setPathParams(apiElement.path, operationElement.parameters)),
    ...
    async.map(testedPaths, function (data, next) {
        ...
        .set('authorization', 'Token=BEA46B8C25A446F1B12E792B61524192')

        .expect(data.status)
        .end(function (err, res) {
            if(err) {
                return next(new Error('Could not find ' + data.method + ' at ' + data.requestUrl + '. Reason: ' + err.message));
            }

            data.result = res.body;
            data.headers = res.headers;
            setTimeout(function() { next(null, data); }, 0)
